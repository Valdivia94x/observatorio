...
export function parseDeudaSOMA(workbook: XLSX.WorkBook): GeneratedGrafica[] {
...
function parseSheet(sheet: XLSX.Sheet, sheetName: string): GeneratedGrafica | null {
  ...
  return {
    titulo: `Deuda del Sistema Operador de Agua en ${muni.display}`,
    tipo: 'stackedBar',
    ubicacion: [muni.ubicacion],
    tablaDatos: {rows: tableRows},
    unidadMedida: 'pesos',
    fuente: 'municipal',
    descripcionContexto: `Deuda total del Sistema Operador de Agua de ${muni.display}, descompuesta en pasivo circulante y no circulante. Fuente: Transparencia Municipal del Ayuntamiento de ${muni.display}.`,
  }
}
