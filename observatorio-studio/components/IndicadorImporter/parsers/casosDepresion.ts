...
// Estructura: fila header con municipios en columnas, filas siguientes [año, ...valores]
export function parseCasosDepresion(workbook: XLSX.WorkBook): GeneratedGrafica[] {
  ...
  // Header row: tiene varios nombres de municipio
  let headerIdx = -1
  ...
  // Recolectar años y valores por municipio
  const anios: string[] = []
  ...
  return muniCols.map((m) => ({
    titulo: `Casos de Depresión Registrados en ${m.display}`,
    tipo: 'bar' as const,
    ...
    fuente: 'salud',
    descripcionContexto: `Casos de depresión registrados anualmente en ${m.display}.`,
  }))
}
