...
// Parse a simple table section (Alumnos or Maestros)
// Returns Map<municipio_lowercase, {periodo, valor}[]>
function parseSimpleTable(
  ...
  // Next row should have municipality names
...
export function parseIndicadoresSecundaria(workbook: XLSX.WorkBook): GeneratedGrafica[] {
  ...
  // Parse sections 1 and 2
  const alumnosData = parseSimpleTable(data, 'Alumnos de Secundaria')
  const maestrosData = parseSimpleTable(data, 'Maestros de Secundaria')

  // Combine Alumnos + Maestros into one chart per ubicación
  if (alumnosData && maestrosData) {
    ...
    for (const municipio of municipios) {
      ...
      graficas.push({
        titulo: `Alumnos y Maestros de Secundaria en ${name}`,
        tipo: 'bar',
        ubicacion,
        tablaDatos,
        unidadMedida: 'unidades',
        fuente: 'sep',
        descripcionContexto: `Alumnos (barras) y maestros (línea) de secundaria en ${name}. Fuente: Sistema de Estadísticas Continuas de Educación, Formato 911.`,
        series: [
  ...
  // Parse section 3: Escuelas por tipo de sostenimiento
  graficas.push(...parseSeccion3Escuelas(data))

  return graficas
}

function parseSeccion3Escuelas(data: (string | number | null)[][]): GeneratedGrafica[] {
  let sectionStart = -1
  for (let i = 0; i < data.length; i++) {
    const row = data[i]
    if (row && row.some((c) => typeof c === 'string' && c.toLowerCase().includes('escuelas de secundaria'))) {
      sectionStart = i
      break
    }
  }
  ...
  for (const block of municipioBlocks) {
    ...
    graficas.push({
      titulo: `Escuelas de Secundaria en ${name}`,
      tipo: 'bar',
      ubicacion,
      tablaDatos,
      unidadMedida: 'unidades',
      fuente: 'sep',
      descripcionContexto: `Número de escuelas de secundaria por tipo de sostenimiento en ${name}.`,
    })
  }
